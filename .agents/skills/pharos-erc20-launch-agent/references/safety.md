...
- **Add FaroSwap liquidity:** run the generated `add-liquidity.mjs` after token deployment. Requires `PRIVATE_KEY`, the deployed token address, enough token/native balance, and explicit confirmation.
...
9. Generate an optional FaroSwap V2 liquidity plan and script.
...
- Treat FaroSwap liquidity as a post-deploy action, not ERC20 contract logic.
